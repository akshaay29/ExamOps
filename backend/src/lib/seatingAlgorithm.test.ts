import { allocateSeats, findAdjacentConflicts, StudentInput, RoomInput } from './seatingAlgorithm'
...
function makeStudents(counts: Record<string, number>): StudentInput[] {
  const out: StudentInput[] = []
  for (const [branch, n] of Object.entries(counts))
    for (let i = 0; i < n; i++) out.push({ id: `${branch}-${i}`, branch })
  return out
}

// Room 5 rows × 6 cols = 30 seats
const ROOM: RoomInput = { id: 'R1', rows: 5, seatsPerRow: 6 }
...
console.log('\n📋 Test 1: No adjacent conflicts (6 branches × 5 = 30, seatsPerRow=6)')
{
  ...
  const { allocations } = allocateSeats(students, [ROOM])
  const conflicts = findAdjacentConflicts(allocations, 'R1')
...
console.log('\n📋 Test 2: Single-branch exam')
{
  ...
  const { allocations, unplaced } = allocateSeats(students, [ROOM])
...
console.log('\n📋 Test 5: Overflow across 2 rooms (5×6=30 + 4×5=20 = 50)')
{
  ...
  const room2: RoomInput = { id: 'R2', rows: 4, seatsPerRow: 5 }
  const { allocations, unplaced } = allocateSeats(students, [ROOM, room2])
  assert(allocations.length === 50, `All 50 placed (got ${allocations.length})`)
  assert(unplaced.length === 0, 'No unplaced')
  const c1 = findAdjacentConflicts(allocations, 'R1')
  const c2 = findAdjacentConflicts(allocations, 'R2')
...
{
  const { allocations, unplaced } = allocateSeats([], [ROOM])
