...
export interface RoomInput {
  ...
  seatsPerRow: number
  layout?: any
  occupiedSeats?: Map<string, { count: number, branches: string[] }> // 'row-col' strings -> occupancy
}

export interface AllocationRecord {
  studentId: string
  roomId: string
  rowNo: number   // 0-indexed
  colNo: number   // 0-indexed
  branch: string
}
...
function pickBranch(
  ...
  if (nonConflicting) return nonConflicting
...
export function allocateSeats(
  ...
  for (const s of students) {
    if (!pool.has(s.branch)) pool.set(s.branch, [])
  ...
  const totalRemaining = () =>
    [...pool.values()].reduce((n, arr) => n + arr.length, 0)

  for (const room of rooms) {
    if (totalRemaining() === 0) break

    // Parse layout for broken capacities
    const layoutCaps: number[][] = Array.from({ length: room.rows }, () => Array(room.seatsPerRow).fill(1))
    if (Array.isArray(room.layout)) {
      for (const item of room.layout) {
        if (item.row < room.rows && item.col < room.seatsPerRow) layoutCaps[item.row][item.col] = item.cap
      }
    }

    const grid: (string | null)[][] = Array.from(
      { length: room.rows },
      () => Array<string | null>(room.seatsPerRow).fill(null)
    )
    ...
    for (let r = 0; r < room.rows; r++) {
      for (let c = 0; c < room.seatsPerRow; c++) {
        ...
        if (layoutCaps[r][c] === 0 || currentOccupancy >= layoutCaps[r][c]) continue
  ...
  for (const arr of pool.values()) unplaced.push(...arr)
 ...
 */
export function findAdjacentConflicts(
  allocations: AllocationRecord[],
  roomId: string
): Array<{ a: AllocationRecord; b: AllocationRecord }> {
  const byRoom = allocations.filter(a => a.roomId === roomId)
  const grid   = new Map<string, AllocationRecord>()
  for (const a of byRoom) grid.set(`${a.rowNo},${a.colNo}`, a)

  const conflicts: Array<{ a: AllocationRecord; b: AllocationRecord }> = []
  for (const a of byRoom) {
    const right = grid.get(`${a.rowNo},${a.colNo + 1}`)
    const below = grid.get(`${a.rowNo + 1},${a.colNo}`)
    if (right && right.branch === a.branch) conflicts.push({ a, b: right })
    if (below && below.branch === a.branch) conflicts.push({ a, b: below })
  }
  return conflicts
}
